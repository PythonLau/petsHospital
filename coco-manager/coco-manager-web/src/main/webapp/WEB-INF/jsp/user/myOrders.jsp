...
<div class="search-result top-distance">
    <h5 align="center" style="font-size: 23px; color: #01AAED"><strong>我的订单</strong></h5>
    <div class="main">
        <div class="container">
            <div class="content">
                <table class="cart-table highlight centered">
                    ...
                    <c:forEach items="${list}" var="order">
                        <tr>
                            ...
                            <td>${order.score}</td>
                            <td>${order.words}</td>
                            <td><img src="${order.image}" height="50" width="50"></td>
                            <td>
                                <c:if test="${order.status == 1}">
                                    <a href="javascript:if(confirm('确实要取消订单吗?'))location='/user/cancelOrder/${order.id}'">
                                        <button class="layui-btn layui-btn-mini layui-btn-normal">取消</button>
                                    </a>
                                </c:if>
                                <c:if test="${order.status == 2}">
                                    <a href="/user/evaluate/${order.id}">
